...
## Push Content Convention

抢单结果推送的正文（content）应包含**店铺名、平台、优惠明细、活动 id**，让用户一条通知即可识别来源，不要只发 `活动{promotionId}`。

- `GrabServiceImpl.buildPushPrefix(config, storeName, promoDetail)` 是抢单结果推送正文的统一前缀构造器，5 处 push（成功/失败/拦截/饭票不足/重试耗尽）复用，形如 `店铺「X」(美团) 满20返15 活动12345`。
- **数据来源优先级**：优先用 `doGrab` 内 `xiaochanHttp.getStorePromotionDetail` 实时查到的 `storeName`/`promoDetail`（更准）；缺失时回退 `grab_config` 快照（`config.getStoreName()`/`config.getPromoDetail()`，由 `AutoGrabServiceImpl.buildBaseEntity` 建任务时写入）。
- **饭票不足分支特殊**：发生在查询活动详情之前，`storeName`/`promoDetail` 尚未赋值，只能用 `config` 快照。
- **平台名映射**：`platformName(storePlatform)` → 1=美团/2=饿了么/3=京东，其它/null 省略平台段。
