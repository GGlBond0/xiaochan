...
- `LotteryHttp`（**App / Android 登录态，抽奖刷任务+开红包**）：`new LotteryHttp()` 后调 `lotteryInfo`/`addLotteryTimes`/`getLotteryProgress`/`onAdViewed`/`receiveExtraLottery`/`lottery`/`userTaskV2`，带 `LotteryAuth`（含 `X-Sivir`），端点 **`gwh`**，body **无 app_id**。
...
### Gotcha: 抽奖执行已实测可纯接口调（推翻旧判定）

> **2026-07-21 更新**：旧判定"执行抽奖 `SilkwormLotteryMobile.Lottery` 被腾讯防水墙挡、需手动过滑块"**已被抓包推翻**。`har/ProxyPin07-20_23_57_52.har` 连续 10 次 `Lottery` 全部 `code=0` 成功返回 prize，无验证码拦截。`LotteryHttp.lottery` 已实现，`/api/lottery/draw` 端点循环消费 `lottery_count`。
>
> 旧判定保留历史背景：前置 `Brs.RiskCheckService.Verify`（腾讯防水墙 TCaptcha）在部分风控触发场景仍可能要求 `ticket/rand_str/check_sum`，但正常账号日常抽奖不触发。若遇 `verify_method!=0` 或验证码错误码，再考虑手动/浏览器过滑块。
