...
## Session 18: 监控自动抢单修复与抢单页活动信息展示

**Date**: 2026-07-15
**Task**: 监控自动抢单修复与抢单页活动信息展示
**Branch**: `main`

### Summary

修复监控自动抢单立即抢分支因executeAt=now被GrabCronScheduler判过期跳过导致永不抢、且ENABLE僵尸永久挡后续命中的bug。改为落auto=1占位不走cron、独立线程池异步doGrab(AUTO)直接执行；防重键改用lastGrabTime已消费语义，所有doGrab结局补写放行。grab_config加auto/活动快照字段，抢单页列表加商家名/优惠/活动时段三列。后端+前端均已部署，清理8个僵尸任务。
...
## Session 19: 自动抢单支持饿了么/京东平台并按平台勾选

**Date**: 2026-07-17
**Task**: 自动抢单支持饿了么/京东平台并按平台勾选
**Branch**: `main`

### Summary

新增饿了么/京东抢单链路(上游 OrderExchange 接口，与美团 GrabPromotionQuota 两套契约)；监控配置新增 grab_platforms 字段按平台勾选启用抢单；doGrab 按平台分叉，AutoGrab 门禁改按集合判断。HAR 抓包确认饿了么走 eleme_status 独立分支、store_type=store_category_sub_type。修复 parsePromotion 饿了么分支漏填字段、store_type 占位取错两个 BUG。已部署生产、DB 列就位、前后端写入链路验证通过。
...
## Session 20: SINGLE模式活动级成功防重修复+部署

**Date**: 2026-07-17
**Task**: SINGLE模式活动级成功防重修复+部署
**Branch**: `main`

### Summary

监控自动抢单SINGLE模式原只有账号级防重(hasPlaceholder带loginStateId),挡不住换号补抢同活动——account=1抢成功后后续命中重新试account=1撞code=11/饭票不足再换号account=2抢成第2个名额(实锤活动118779162)。修复:AutoGrabServiceImpl SINGLE分支入口加grabbedSuccessPromotionIds活动级成功预检(当天auto=true+status=DISABLE+lastResult LIKE 成功%即整组跳过),ALL模式不动。本地编译通过,split分片scp部署绕开scp卡死坑,md5校验一致,服务重启active。spec补SINGLE活动级防重约定。遗留:07-16-addr-login-state-select仍in_progress(非本会话)。
...
## Session 21: STORE_KEYWORD分钟级去重收尾

**Date**: 2026-07-20
**Task**: STORE_KEYWORD分钟级去重收尾
**Branch**: `main`

### Summary

确认StoreTask.java改动已于07-19 03:16编译进线上jar并生效(经javap核对class含dedupMinutesOf/findPushedWithinMinutes/cleanupExpired;生产日志configId=5斑斓包点07-20 00:22重新推送成功,验证永久去重已解除)。补spec database-guidelines去重条目,git提交,归档任务。
...
## Session 22: A-6死代码清理+B-10 STORE_ACTIVITY去命中即停

**Date**: 2026-07-20
**Task**: A-6死代码清理+B-10 STORE_ACTIVITY去命中即停
**Branch**: `main`

### Summary

A-6:删XiaoChanController apply/book/ignore三空接口+BookVO/IgnoreStoreVO+前端handleApply/handleIgnore死函数,保留handleBook。B-10:删StoreTask.afterSuccess的toggleStatus(DISABLE),STORE_ACTIVITY命中不再永久停,靠checkRepeat当天去重+跨天自动重置实现再次通知,对齐MINIMUM_PAY。本地编译通过,jar已rsync/scp部署上线22:36,线上javap确认StoreTask.class无afterSuccess,服务active。
...
| Hash | Message |
|------|---------|
| `ceb93aa93c55509e3ec32b77092c11745739ec9a` | (see git log) |
...
## Session 25: XiaoChanServiceImpl NPE+配置类事务(B-4/6/7)

**Date**: 2026-07-20
**Task**: XiaoChanServiceImpl NPE+配置类事务(B-4/6/7)
**Branch**: `main`

### Summary

B-4:XiaoChanServiceImpl distance null用nullsLast+name null防御(3处)。B-6:ProxyConfig/MerchantBlacklist updateConfig加@Transactional。B-7:UserServiceImpl register加事务。核实后不改:B-5(sendOne吞异常彻底修需sendMessage返回值冒泡重构)、B-9(silkId Integer类型约束不越界)。jar部署上线23:01 MD5一致active。
...
| Hash | Message |
|------|---------|
| `5b346aa95328860b1b66f993847197b815ba78cb` | (see git log) |
