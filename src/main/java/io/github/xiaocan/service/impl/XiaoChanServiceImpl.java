...
public class XiaoChanServiceImpl implements XiaoChanService {
     ...
     * 3km 筛选半径（米）
     */
    private static final int DISTANCE_3KM = 3000;
    ...
    public List<StoreInfo> query(QueryListVO queryListVO) {
        ...
        if (StringUtils.isNotBlank(queryListVO.getName())) {
            ...
            //搜索结果同样应用距离排序与 3km 筛选，保持与浏览列表一致
            sortStoreList(result, queryListVO.getOrderType());
            result = filter(result, queryListVO);
        }else{
            boolean needFullScan = (queryListVO.getOrderType() != null && queryListVO.getOrderType() != 1)
                    || Boolean.TRUE.equals(queryListVO.getWithin3km());
            if (needFullScan) {
                //排序不为默认 或 开启 3km 筛选，则获取所有活动再排序过滤
                if (pageNum > 0) {
                    return Collections.emptyList();
                }
    ...
    private List<StoreInfo> filter(List<StoreInfo> list, QueryListVO queryListVO) {
        ...
        // 3km 内筛选：仅保留距离 <= 3000 米的门店，无 distance 的门店排除
        if (Boolean.TRUE.equals(queryListVO.getWithin3km())) {
            list = list.stream()
                    .filter(storeInfo -> storeInfo.getDistance() != null && storeInfo.getDistance() <= DISTANCE_3KM)
                    .toList();
        }
