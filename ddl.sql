...
-- user 表扩展：小蚕登录态（X-Sivir/X-Teemo/X-Session-Id/X-Nami）
ALTER TABLE `user`
...
-- 抢单配置表
DROP TABLE IF EXISTS `grab_config`;
CREATE TABLE `grab_config` (
    `id`               INT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id`          INT NOT NULL COMMENT '用户ID',
    `location_id`      BIGINT NULL DEFAULT NULL COMMENT '位置信息ID(为空则用入参经纬度)',
    `promotion_id`     INT NOT NULL COMMENT '要抢的活动id(当天有效)',
    `silk_id`          INT NULL DEFAULT 0 COMMENT 'silk_id',
    `store_platform`   INT NOT NULL DEFAULT 1 COMMENT '平台,默认1',
    `if_advance_order` TINYINT NOT NULL DEFAULT 0 COMMENT '是否预售',
    `cron`             VARCHAR(50) NULL DEFAULT NULL COMMENT '定时抢单cron(6位含秒),空=仅手动/一次性',
    `execute_at`       DATETIME(3) NULL DEFAULT NULL COMMENT '一次性精确执行时间,命中后停用',
...
CREATE TABLE `grab_history` (
    ...
    `resp_code`          INT NULL DEFAULT NULL COMMENT '小蚕返回code(0成功,4未开始,6抢完)',
...
-- 回滚：
-- DROP TABLE `grab_history`;
-- DROP TABLE `grab_config`;
-- ALTER TABLE `user`
...
CREATE TABLE `merchant_blacklist_config`  (
  ...
  `keywords`    TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '关键字规则，一行一条；行内 & 表示 AND；多行为 OR；大小写不敏感子串包含',
  `create_time` DATETIME NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted`     TINYINT(1) NULL DEFAULT 0 COMMENT '逻辑删除标志',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '商家名称关键字黑名单全局配置表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for lottery_auth  (小蚕霸王餐刷任务 App(Android) 登录态，独立于 grab_login_state)
-- ----------------------------
...
CREATE TABLE `lottery_auth`  (
  ...
  `city_code`   INT NULL DEFAULT NULL COMMENT 'x-City 城市码,如 440111(抓包解析)',
